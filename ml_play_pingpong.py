...
def ml_loop(side: str):
    ...
    filename = path.join(path.dirname(__file__),"save/clf.SVR.pickle")  #讀這隻檔案的位置
    with open(filename, 'rb') as file:
        clf = pickle.load(file)

    def move_to(player, pred) : #move platform to predicted position to catch ball 
        if player == '1P':
            if scene_info["platform_1P"][0]+20  > (pred-20) and scene_info["platform_1P"][0]+20 < (pred+20): return 0 # NONE
            elif scene_info["platform_1P"][0]+20 <= (pred-20) : return 1 # goes right
            else : return 2 # goes left
        else :
            if scene_info["platform_2P"][0]+20  > (pred-20) and scene_info["platform_2P"][0]+20 < (pred+20): return 0 # NONE
            elif scene_info["platform_2P"][0]+20 <= (pred-20) : return 1 # goes right
            else : return 2 # goes left
    ...
    while True:
        ...
        data = [[scene_info["ball"][0],scene_info["ball"][1],scene_info["ball_speed"][0],scene_info["ball_speed"][1],scene_info["blocker"][1]]]
        feature = pd.DataFrame(data,columns=["ball_x","ball_y","ball_speed_x","ball_speed_y","blocker_x"])
        print (feature)
